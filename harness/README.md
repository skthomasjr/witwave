...
| `ADHOC_RUN_AUTH_TOKEN`        | _(unset)_                    | Bearer token required on `/heartbeat/run`, `/jobs/{n}/run`, `/tasks/{n}/run`, `/triggers/{n}/run`. Ad-hoc run handlers honour the `backends_ready` warmup shield — requests during startup return 503 rather than racing the executor (#925, #955). Advertised in `/.well-known/agent-runs.json` (#956).                                              |
| `CONVERSATIONS_AUTH_TOKEN`    | _(unset)_                    | Bearer token required on `/conversations`, `/trace`, `/mcp`. Empty token logs a startup warning (#517).                                                                                                                                                                                                                                               |
