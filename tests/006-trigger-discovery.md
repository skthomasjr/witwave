...
The response must be a JSON array. Verify that it contains at least one entry with `"endpoint": "ping"` and that each entry has the fields `endpoint`, `name`, `methods`, and `session_id`.

The test passes if the response is a valid JSON array containing an entry with `"endpoint": "ping"`.
The test fails if the endpoint is unreachable, returns non-JSON, returns an empty array, or the ping entry is missing.
