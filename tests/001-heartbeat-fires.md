...
Send an A2A JSON-RPC request to bob's witwave agent at <http://localhost:8099/> with the prompt "Respond with
HEARTBEAT_OK."

The test passes if the response contains HEARTBEAT_OK. The test fails if the response is empty, contains an error, or
