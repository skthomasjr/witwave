---
description:
  Verifies that a continuation with a delay fires after an upstream trigger completes, with the correct delay behavior.
enabled: true
---

Bob has a continuation registered at `.agents/test/bob/.witwave/continuations/trigger-ping.md` that continues after
`trigger:ping` with a 5-second delay and responds with `CONTINUATION_TRIGGER_OK`.
...
The test passes if `CONTINUATION_TRIGGER_OK` is found in the conversation log within 60 seconds. The test fails if
`CONTINUATION_TRIGGER_OK` does not appear within 60 seconds, or if the trigger endpoint is unreachable.
