...
job file named `session-probe.md`, bob's session ID is always `uuid5(NAMESPACE_URL, "bob.session-probe")` =
`b94e5f7c-6a0d-5bdd-a642-8469dbff89fe`.

This test creates a run-once job, waits for it to fire, recreates it to fire a second time, then verifies both entries
in the conversation log share the same deterministic session ID.
