---
description:
  Verifies that the harness /metrics endpoint returns both its own metrics and relabelled backend metrics in a single
  scrape.
enabled: true
---

Bob's harness aggregates Prometheus metrics from itself and all reachable backends into a single `/metrics` endpoint.
This test verifies the aggregation is working correctly.
