...
2. Read `~/.nyx/manifest.json` to find the target agent's URL
...
URL=$(cat ~/.nyx/manifest.json | python3 -c "import sys,json; team=json.load(sys.stdin)['team']; match=[a['url'] for a in team if a['name']=='$TARGET']; print(match[0] if match else '')")
