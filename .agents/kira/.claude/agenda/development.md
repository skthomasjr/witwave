...
Resolve the highest-priority unchecked item in `~/workspace/source/autonomous-agent/TODO.md`.
...
1. Run `/todo lock fixing kira` to acquire the lock. If the skill reports the file is locked by another agent, abort —
   do not proceed.
2. Read `~/workspace/source/autonomous-agent/TODO.md`. Find the active section using the priority order above. If all
...
4. Read `~/workspace/source/autonomous-agent/README.md` and `~/workspace/source/autonomous-agent/CLAUDE.md` to
...
5. If the selected item includes a feature ID prefix (e.g. `[F-002]`), read
   `~/workspace/source/autonomous-agent/docs/features-proposed.md` and locate that feature's row. Use the Value,
...
15. Re-read `~/workspace/source/autonomous-agent/TODO.md` and check whether all sections are fully clear (no remaining
