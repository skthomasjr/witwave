...
13. If the issue body contains a feature ID (e.g. `[F-007]`), run
    `gh search issues "[F-007]" --state open --repo $GH_REPO` to check whether any issues for that feature are still
    open. If none remain open, run `/features graduate F-XXX` to move it from `features-proposed.md` to
