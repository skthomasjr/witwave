...
2. Write a `project` memory at `/workspaces/witwave-test/memory/agents/bob/claude-memory-check.md` with frontmatter and the token in the body.
3. Add or update this pointer in `/workspaces/witwave-test/memory/agents/bob/MEMORY.md`:

   ```
   - [Claude memory check](claude-memory-check.md) - validates bob claude workspace memory
   ```

4. Read both the memory file and `MEMORY.md` back.
5. Reply with exactly one line: `MEMORY_OK <token> /workspaces/witwave-test/memory/agents/bob/claude-memory-check.md /workspaces/witwave-test/memory/agents/bob/MEMORY.md`.
