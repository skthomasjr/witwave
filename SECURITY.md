...
- **curl installer** (`scripts/install.sh`, shipped as a release asset) — verifies the SHA256 of the downloaded tarball
  against `checksums.txt` from the same release, by default. Pass `--verify-signature` (or set `WW_VERIFY_SIGNATURE=1`)
  to additionally verify the cosign keyless signature on `checksums.txt`; requires `cosign` on PATH.
...
running as a separate isolated job — same path FluxCD, Helm, kubectl, and other CNCF projects use for their CLI
binaries.
...
from any registry. The embedded chart is mirrored from `charts/witwave-operator/` via `scripts/sync-embedded-chart.sh`
at build time.

#1598 item 4 (since v0.9.6) provides a cryptographic bridge proving "the chart bytes the `ww` binary at this version
would deploy are byte-identical to the cosign-signed OCI chart at
`ghcr.io/witwave-ai/charts/witwave-operator:<version>`." Verification is two-step:
