...
// WitwaveWorkspace name length cap. The CRD bounds the field via DNS-1123
// pattern + 63-char limit on the operator side; this matches.
const maxWitwaveWorkspaceNameLen = 63
...
// ValidateName enforces the naming rules for WitwaveWorkspace CRs. Same shape
// as agent.ValidateName but bumped to 63 chars because WitwaveWorkspace names
...
func ValidateName(name string) error {
	...
	if len(name) > maxWitwaveWorkspaceNameLen {
		return fmt.Errorf(
			"workspace name %q is %d characters; maximum is %d (DNS-1123 label limit)",
			name, len(name), maxWitwaveWorkspaceNameLen,
		)
	}
...
// WitwaveWorkspace CRD's WitwaveWorkspaceVolume.Name field.
