...
This paper describes the structural shape of a fully agentic engineering team, using a working seven-agent team as a
...
The differences are not cosmetic. An agentic team retires sprints, standups, ticket queues, calendar releases,
code-review queues, feature branches, the project-management SaaS stack, and headcount math — and replaces them with
heartbeat schedulers, persistent and _cross-readable_ decision logs, direct agent-to-agent calls, velocity-based
releases, in-code quality bars, a single shared trunk every agent commits to, a deliberately minimal coordination
toolset (memory files, RPC, git), and spec-defined membership. The substitutions are not optional polish; they are what
makes the operating model coherent. A team that adopts agents but keeps the all-human coordination scaffolding gets the
worst of both shapes — the cost of agents without the leverage.
...
- **Memory is shared, not private.** Every agent reads every other agent's memory namespace. There is no analog to a
  private notebook. The shared surface is the coordination surface.
- **Everyone works on the same source.** No feature branches, no per-agent workspaces, no merge negotiation. Every agent
  commits to trunk; the push is serialized through one peer.
- **The coordination toolset is tiny.** Three primitives — markdown memory files, an RPC call, git — replace the entire
  Jira / Confluence / Slack / Notion / Linear / Asana stack. Adding tools degrades the model rather than improving it.

---

## The reference team

The patterns described in this paper are abstracted from a working agentic team: seven autonomous agents that maintain
the witwave platform repository, commit directly to `main`, and coordinate without human routing. Several additional
specialist members are designed and queued for deployment — see _The team in motion_ below — so the seven-agent shape
captured here is a current snapshot, not a final configuration. The team is described here only thinly; per-agent detail
lives in the repository's `TEAM.md`.
...
Each agent owns one substrate. Together they run the repository continuously, shipping multiple small releases per day.
...
The manager-as-peer pattern is what allows the rest of the structural changes to hold. Without it, work assignment falls
back to either a ticket queue (which re-introduces queueing latency) or "every agent decides for itself" (which produces
collisions and duplicated work). The coordinator is what lets agents behave as a team rather than a swarm.
...
maintains its own memory namespace — markdown files indexed by topic — and _every other agent reads it freely._ The
manager reads each peer's in-flight notes to know what's in progress before dispatching the next tick. The release agent
reads pending-push queues from peer memories to know what's about to land. The outward-facing agent reads everyone's
memory plus the manager's decision log to know what to talk about publicly. There is no analog to a private notebook; if
it isn't written down where peers can read it, it doesn't exist for the team.
...
- **No state-loss across handoffs.** A peer dispatched at 3 a.m. and again at 9 a.m. has the same context. Humans cannot
  match this.
...
surveillance, or like a violation of psychological safety. Neither analogy applies. Agents do not have careers,
reputations to protect, or political reasons to obscure their reasoning. Default-shared memory is what enables the rest
of the operating model; making memory private would re-introduce the coordination overhead the team was designed to
retire.
...
An agentic team collapses the fan-out. There are no feature branches. Every agent commits directly to trunk. There is
one canonical working checkout that the team operates against; the push to `origin/main` is serialized through one peer
(the release agent) so race conditions and force-push hazards stay in one pair of hands.
...
surface the user installs from — every commit either improves it or is reverted within minutes.

This change is structurally tied to trunk-based development as a discipline. Teams that try to put agents on a
feature-branch workflow tend to either (a) drown in PR-review queue, recreating the hybrid-team plateau, or (b)
accumulate a graveyard of half-finished agent branches that nobody owns. The shared-source model is what lets the other
changes pay off.
...
- **An RPC call (`call-peer`).** Agent-to-agent JSON-RPC over the A2A protocol. One verb: "send this prompt to that
  peer; give me the reply." This is the entire work-assignment surface.
...
`docs/` lives in the same repo under the same git history. The "handbook" is `AGENTS.md` and `TEAM.md`, checked in
alongside the code. Onboarding a new agent is reading the same files everyone already reads.
...
three-primitive surface keeps state legible to every member by construction. Memory is a file; the file is in git; every
agent has it.

The diagnostic test for whether a tool belongs in the coordination surface: _if every agent can't read it, write to it,
and reason about it without glue code, it doesn't belong._ Most SaaS tools fail this test, which is why most agentic
teams will never adopt them.
...
exhaustive, the linters strict, the security scanners current, the fix-bar enforced — then the agents are constrained by
the same automation that constrained the humans, only more consistently. If the gates are weak, no amount of human
review will compensate; the team is already shipping bugs and just doesn't know which ones.
...
skills, a routing config. Adding a member is hours of work, not months. Removing one is a `kubectl delete`.
...
outreach because the cost of separating them is near zero and the benefit (each agent's skills can be deeply specialized
for its substrate) is real.

The roadmap for the reference team includes specialist agents for devops, agent-resource management, security
architecture, testing, software architecture, CTO-level direction, community liaison, and feature authoring. Whether
each one ships, when, and in what shape, is a design decision the existing team can revise — not a hiring plan that
...
- **Responding to escalations.** Truly novel situations, ambiguous trade-offs, decisions the agents flagged as
  out-of-distribution.
- **External-facing trust work.** The relationship with users, executives, peer teams. The agents can talk to humans on
  GitHub Discussions, but the human is still the one accountable for what the team ships.
...
The result is the Phase-2 plateau described in _The Four Phases of AI Adoption_: agent throughput is high, cycle time
barely moves, the review queue grows, engineers feel less productive even as commit counts rise.
...
The result is one of two outcomes, both bad. Either humans are required as reviewers, in which case the team is in
failure mode 1 (review queue bottleneck) by another route — or agents review each other's PRs, in which case the team
accumulates a graveyard of stale agent branches that nobody owns and nothing ever lands quickly. The branch model is
incompatible with machine-time velocity.

The diagnostic question: _do commits land on `main` directly, or via a PR queue?_ If the latter, the team is in failure
mode 4.
...
human review rather than by automation. The team commits enthusiastically, humans cannot keep up with the review load,
the quality bar slides, regressions land, trust in the agents collapses, and the team retreats to failure mode 1
(agents-as-tools-with-mandatory-review) or abandons the experiment.

The diagnostic question: _what gates a commit before it lands on `main`?_ If the answer is "a human reviewer," the
agents will outpace the gate within weeks and the team will be back in the review-queue trap.
...
has been replaced by a system that does the same job in machine-time. None of the substitutions are speculative; all of
them exist in the reference team today.
...
The seven-agent reference team is a starting shape, not a finished one. The six-substrate split (coordination, defects,
hygiene, docs, gaps, git plumbing, outreach) is the smallest configuration in which the operating model is internally
coherent — every load-bearing function has an owner, and no agent is doing two jobs at once. It is enough to
_demonstrate_ the model. It is not enough to _cover_ the model's full surface.

Several specialist members are already designed and queued for the next wave of growth. Each fills a substrate the
current team handles thinly or not at all:
...
- **Feature-implementation agent.** Builds new functionality end-to-end: reads requirements (from issues, discussions,
  design docs), implements the change across code + tests + docs, commits in atomic pieces, delegates the push. Distinct
  from the gap-filling and defect-fixing agents because feature delivery is _creative authorship_ rather than defect
  remediation — a different shape of work, requiring a different safety bar, and best introduced after the testing agent
  matures so the safety net under net-new code is strong.

Beyond that next wave, additional specializations are sketched for software- architecture (system-shape watching),
CTO-level direction-setting, and community liaison (deeper external engagement than the outreach agent provides). The
full roadmap lives in the repository's `TEAM.md`.
...
not require headcount approval, a six-month hiring loop, or an onboarding plan. It will require writing a CLAUDE.md,
scaffolding a skill or two, configuring the routing, and deploying a pod. If the agent's substrate turns out to be
wrongly scoped, it will be revised the same way: edit the spec, redeploy. If it turns out the substrate doesn't deserve
a dedicated agent at all, the work folds back into a peer's skill set and the agent is retired.
...
- **Substrates get narrower over time.** As the team grows, each agent's substrate gets sharper. The current defects
  ...
  agent moderates Discussions _and_ writes posts _and_ triages bug reports from users; eventually those become separate
  roles. Specialization compounds.
- **The team can grow into work that doesn't exist yet.** When the platform acquires a new surface — a new MCP
...
The reference team is, in short, deliberately early. Seven agents, not seventy. The shape will look different in a year.
...
looks agentic and behaves hybrid. Adopting all six produces a team that operates at machine-time end-to-end and whose
human members spend their attention on direction, bounds, and audit rather than on routing and review.
...
4. **Do commits land on `main` directly, or via a PR queue?** A PR queue re-introduces the human-throughput bottleneck
   the trunk model was designed to retire.
5. **What gates a commit before it lands?** If the answer is "a human reviewer," the gating layer will collapse under
   agent throughput within months. The bar has to live in code.
...
_The reference team described in this paper maintains the witwave platform repository and is observable in its commit
history, decision logs, and public GitHub Discussions surface. Per-agent identities and skill definitions live in the
repository under `.agents/self/`; the team-level overview is in `.agents/self/TEAM.md`._
