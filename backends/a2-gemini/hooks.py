...
Scope caveat — a2-gemini does not yet have native tool-calling support in
...
def _bump_config_error(reason: str) -> None:
    ...
    Mirrors the a2-claude reporter pattern. Any failure here is swallowed so
...
def load_hooks_config_sync() -> list:
    ...
    Mirrors a2-claude's ``_load_hooks_config_sync`` so the executor's watcher
