...
// invariants for #1112:
//
//  1. It grows up to nyxv1alpha1.ReconcileHistoryMax entries without
//     dropping any.
...
func TestAppendReconcileHistory_GrowsThenRotates(t *testing.T) {
	...
	const max = nyxv1alpha1.ReconcileHistoryMax
	var ring []nyxv1alpha1.ReconcileHistoryEntry
	...
	for i := 0; i < max; i++ {
		...
		if last.Phase != nyxv1alpha1.ReconcileHistoryPhaseSuccess {
			t.Fatalf("append %d: phase=%q want=%q", i, last.Phase, nyxv1alpha1.ReconcileHistoryPhaseSuccess)
		}
	...
	last := ring[len(ring)-1]
	if last.Phase != nyxv1alpha1.ReconcileHistoryPhaseError {
		t.Fatalf("rotated-in phase=%q want=%q", last.Phase, nyxv1alpha1.ReconcileHistoryPhaseError)
	}
...
func TestAppendReconcileHistory_TruncatesLongMessages(t *testing.T) {
	...
	long := strings.Repeat("x", nyxv1alpha1.ReconcileHistoryMessageMax*2)
	ring := appendReconcileHistory(nil, time.Now(), time.Millisecond, errors.New(long))
	if len(ring) != 1 {
		t.Fatalf("expected 1 entry, got %d", len(ring))
	}
	msg := ring[0].Message
	if len(msg) > nyxv1alpha1.ReconcileHistoryMessageMax {
		t.Fatalf("message len=%d exceeds cap=%d", len(msg), nyxv1alpha1.ReconcileHistoryMessageMax)
	}
