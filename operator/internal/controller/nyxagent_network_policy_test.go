...
func npAgent(name string) *nyxv1alpha1.NyxAgent {
...
func TestBuildNetworkPolicy_EnabledEmptyIngressDeniesAll(t *testing.T) {
	a := npAgent("iris")
	a.Spec.NetworkPolicy = &nyxv1alpha1.NetworkPolicySpec{
		Enabled: true,
...
func TestBuildNetworkPolicy_DashboardPeerAllowed(t *testing.T) {
	...
	a.Spec.NetworkPolicy = &nyxv1alpha1.NetworkPolicySpec{
		Enabled: true,
		Ingress: &nyxv1alpha1.NetworkPolicyIngressSpec{
...
func TestBuildNetworkPolicy_EgressOpenByDefault(t *testing.T) {
	a := npAgent("iris")
	a.Spec.NetworkPolicy = &nyxv1alpha1.NetworkPolicySpec{Enabled: true}
	np := buildNetworkPolicy(a)
	for _, pt := range np.Spec.PolicyTypes {
