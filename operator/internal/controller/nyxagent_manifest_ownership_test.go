...
func TestBuildManifestConfigMapMultiOwner(t *testing.T) {
	newAgent := func(name string, uid string) *nyxv1alpha1.NyxAgent {
		return &nyxv1alpha1.NyxAgent{
			...
			Spec: nyxv1alpha1.NyxAgentSpec{Port: 8000},
		}
	}
	...
	cm, hash := buildManifestConfigMap(iris, []*nyxv1alpha1.NyxAgent{iris, nova, kira}, members)
	...
	for i, ref := range cm.OwnerReferences {
		...
		if ref.Kind != "NyxAgent" {
			t.Errorf("ownerRef[%d]: Kind=%q, want NyxAgent", i, ref.Kind)
		}
	}
...
func TestBuildManifestConfigMapHashSensitiveToMembership(t *testing.T) {
	mk := func(name, uid string) *nyxv1alpha1.NyxAgent {
		return &nyxv1alpha1.NyxAgent{
			ObjectMeta: metav1.ObjectMeta{
				Name:      name,
				Namespace: "nyx",
	...
	_, hashA := buildManifestConfigMap(mk("iris", "uid-a"), []*nyxv1alpha1.NyxAgent{mk("iris", "uid-a")}, members)
	_, hashB := buildManifestConfigMap(mk("iris", "uid-b"), []*nyxv1alpha1.NyxAgent{mk("iris", "uid-b")}, members)
	if hashA == hashB {
	...
	_, hashTwo := buildManifestConfigMap(
		mk("iris", "uid-a"),
		[]*nyxv1alpha1.NyxAgent{mk("iris", "uid-a"), mk("nova", "uid-n")},
		twoMembers,
	)
	if hashA == hashTwo {
...
func TestManifestVolumeIsOptional(t *testing.T) {
	vol, _ := manifestVolumeAndMount("nyx-manifest-alpha")
