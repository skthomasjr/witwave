...
import (
	"crypto/sha1" //nolint:gosec // SHA-1 is used for short, non-cryptographic volume-name disambiguation — matches the chart's `witwave.gmVolumeName` helper so operator-rendered pods carry the same volume names chart-rendered pods do.
	"encoding/hex"
	"fmt"
	"sort"
...
//
// Mirrors charts/witwave/templates/deployment.yaml + _helpers.tpl so operator-
...
const (
	...
	gitScriptMountPath = "/witwave-scripts"

	// gitMappingsMountPath is the root where per-context mapping TSV
	// files are mounted. Matches the chart: `/witwave-mappings/agent` for
	// the agent-scoped mappings, `/witwave-mappings/<backend>` for each
	...
	gitMappingsMountPath = "/witwave-mappings"
...
// ConfigMap. Byte-identical to charts/witwave/templates/configmap-git-mappings.yaml
...
set -e
MAPPINGS_DIR="${1:-/witwave-mappings}"
...
func hasGitMappings(agent *witwavev1alpha1.WitwaveAgent) bool {
...
// gmVolumeName mirrors the chart's `witwave.gmVolumeName` helper so operator-
...
// script. One CM per WitwaveAgent so OwnerReferences GC works without
// cross-CR reasoning.
func gitSyncScriptCMName(agent *witwavev1alpha1.WitwaveAgent) string {
...
// "agent" label under /witwave-mappings) or a backend name for a per-backend CM.
func gitMappingsCMName(agent *witwavev1alpha1.WitwaveAgent, backendName string) string {
	if backendName == "" {
...
// to the built-in default when unset. Mirrors the chart's `witwave.gitSyncImage`
// helper — per-entry override takes precedence.
func gitSyncImage(entry witwavev1alpha1.GitSyncSpec, appVersion string) string {
	if entry.Image != nil && entry.Image.Repository != "" {
		return imageRef(*entry.Image, appVersion)
	}
	return imageRef(witwavev1alpha1.ImageSpec{Repository: gitSyncImageRepository}, appVersion)
}
...
func buildGitSyncScriptConfigMap(agent *witwavev1alpha1.WitwaveAgent) *corev1.ConfigMap {
...
func buildGitMappingsConfigMaps(agent *witwavev1alpha1.WitwaveAgent) []*corev1.ConfigMap {
...
func buildMappingsCM(agent *witwavev1alpha1.WitwaveAgent, backendName string, mappings []witwavev1alpha1.GitMappingSpec) *corev1.ConfigMap {
	var tsv string
...
func gitSyncVolumes(agent *witwavev1alpha1.WitwaveAgent) []corev1.Volume {
...
// Mirrors the chart's `witwave.gitMappingMounts` helper.
func gitMappingScriptMounts(agent *witwavev1alpha1.WitwaveAgent) []corev1.VolumeMount {
...
func gitSyncInitContainers(agent *witwavev1alpha1.WitwaveAgent, appVersion string) []corev1.Container {
	if len(agent.Spec.GitSyncs) == 0 {
		return nil
	}
	...
	if hasGitMappings(agent) {
		inits = append(inits, corev1.Container{
			Name:    "git-map-init",
			Image:   gitSyncImage(witwavev1alpha1.GitSyncSpec{}, appVersion),
...
func gitSyncSidecarContainers(agent *witwavev1alpha1.WitwaveAgent, appVersion string) []corev1.Container {
	if len(agent.Spec.GitSyncs) == 0 {
		return nil
	}
...
func agentGitMappingMounts(agent *witwavev1alpha1.WitwaveAgent) []corev1.VolumeMount {
	if len(agent.Spec.GitSyncs) == 0 {
		return nil
	}
	mounts := []corev1.VolumeMount{{
...
func backendGitMappingMounts(agent *witwavev1alpha1.WitwaveAgent, b witwavev1alpha1.BackendSpec) []corev1.VolumeMount {
	if len(agent.Spec.GitSyncs) == 0 {
		return nil
	}
	mounts := []corev1.VolumeMount{{
